...
#ifdef DAKOTA_OPTPP
  virtual void printStatus(const char * msg) { printStatus(msg, true);}
#else
  virtual void printStatus(const char * msg) { printStatus(msg, false);}
  /// Print the status of optimizer at Xc, but not Xc 
#endif

  void printStatus(const char * msg, bool printXc);
