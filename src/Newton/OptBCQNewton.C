...
const static char* class_name = "OptBCQNewton";
...
void OptBCQNewton::printStatus(const char *s) // set Message
{
  NLP1* nlp = nlprob();
...
double OptBCQNewton::computeMaxStep(ColumnVector &sk)
{
  ...
  {
	  if (debug_)
	    *optout << "computeMaxStep Hit an upper constraint for variable " << i << "\n";
	}
      }
      //else if (sk(i) < 0.0e0) {
      else if (sk(i) < -1e-15) {
        ...
        if (delta <= feas_tol) {
	  if (debug_)
	    *optout << "computeMaxStep Hit a  lower constraint for variable " << i << "\n";
	}
      }
      else delta=FLT_MAX;
