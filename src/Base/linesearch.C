...
int linesearch(NLP1* nlp, ostream *optout, 
 ...
 *****************************************************************************/
  ...
  if (expensive_function)
    ...
    stpmax = stpmax / snorm;
    if (stpmax<stpmin) {
      cerr << "Warning: in linesearch(): stpmax/stpnorm < stpmin\n";
      stpmax = 10*stpmin;      
    }
    if(*step_length>=stpmax) *step_length=(stpmax+stpmin)/2;
    step_type = mcsrch(nlp, search_dir, optout, step_length, itnmax, ftol, xtol, gtol, stpmax, stpmin);
  }
  return(step_type);
}
